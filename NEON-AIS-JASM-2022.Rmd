...
In this exercise, we will want data from only one NEON field site, Walker Branch 
TN (WALK) from July, 2019. 
...
mergedData2<-merge(NSW_15_minute,csd_continuousDischarge,by.x="endDateTime",by.y="endDate",all.x=F,all.y=F)
...
mergedData2<-merge(NSW_15_minute,csd_15_minute,by.x="endDateTime",by.y="endDate",all.x=F,all.y=F)

ggplot(data = mergedData2, 
       aes(x = maxpostDischarge, y = surfWaterNitrateMean)) +
  geom_point(color="green") + 
  ggtitle("WALK NO3-N vs. Q") + 
  xlab("Q (L/s)") + 
  ylab("NO3-N (uM)")
```
